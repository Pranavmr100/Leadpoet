...
def fetch_company_domains(industry, max_domains=100):
    """
    WE're fetching the company domains from an open-source list like the one above and filtering by industry.
    """
    try:
        ...
        # If there's not enough matches, fill with other domains
        if len(domains) < max_domains:
            for company in data:
                if len(domains) >= max_domains:
                    break
